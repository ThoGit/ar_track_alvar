...
namespace alvar
{
  ...
  {
  protected:
    void VisualizeMarkerPose(IplImage *image, Camera *cam, double visualize2d_points[12][2],
                             CvScalar color = CV_RGB(255, 0, 0)) const;

    virtual void VisualizeMarkerContent(IplImage *image, Camera *cam, double datatext_point[2],
                                        double content_point[2]) const;

    virtual void VisualizeMarkerError(IplImage *image, Camera *cam, double errortext_point[2]) const;

    bool UpdateContentBasic(std::vector<Point<CvPoint2D64f> > &_marker_corners_img, IplImage *gray, Camera *cam,
                            int frame_no = 0);
  ...
  public:
    ...
    /** \brief Compares the marker corners with the previous match.
     */
    void CompareContent(std::vector<Point<CvPoint2D64f> > &_marker_corners_img, IplImage *gray, Camera *cam,
                        int *orientation) const;

    /** \brief Updates the \e marker_content from the image using \e Homography
     */
    virtual bool UpdateContent(std::vector<Point<CvPoint2D64f> > &_marker_corners_img, IplImage *gray, Camera *cam,
                               int frame_no = 0);

    /** \brief Updates the markers \e pose estimation
     */
    void UpdatePose(std::vector<Point<CvPoint2D64f> > &_marker_corners_img, Camera *cam, int orientation,
