...
template
class ALVAR_EXPORT alvar::MarkerDetector<alvar::Marker>;

template
class ALVAR_EXPORT alvar::MarkerDetector<alvar::MarkerData>;

template
class ALVAR_EXPORT alvar::MarkerDetector<alvar::MarkerArtoolkit>;
                                 ...
                                 bool update_pose)
  {
    ...
    switch (labeling_method)
    {
      case CVSEQ :
        ...
        if (labeling == NULL)
          labeling = LabelingPtr(new LabelingCvSeq());

        LabelingCvSeq* p_labeling_cv_seq = dynamic_cast<LabelingCvSeq*>(labeling.get());
        if (p_labeling_cv_seq != NULL)
        {
          p_labeling_cv_seq->SetOptions(detect_pose_grayscale);
        }
        else
        {
          std::logic_error("Throw cast from Labeling -> LabelingCvSeq failed");
        }
      ...
      for (size_t ii = 0; ii < _track_markers_size(); ii++)
      {
        ...
        if (track_error <= max_track_error)
        {
          ...
          mn->UpdateContent(blob_corners[track_i], gray,
                            cam);    //Maybe should only do this when kinect is being used? Don't think it hurts anything...
